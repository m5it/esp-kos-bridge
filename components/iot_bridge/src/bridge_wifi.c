                              ...
                              const char *bssid)
{
    ...
    if (mode & WIFI_MODE_STA) {
		ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_set() MODE_STA!\n");
    ...
    if (mode & WIFI_MODE_AP) {
		ESP_LOGI(TAG, "bridge_wifi.c ->  esp_bridge_wifi_set() MODE_AP!\n");
		//
...
{
	ESP_LOGI(TAG, "DEBUG wifi_event_sta_disconnected_handler() STARTING.\n");
...
{
	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_init() STARTING.\n");
    if (s_wifi_event_group) {
		ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_init() LooKS OK.\n");
        return ESP_OK;
    }

    s_wifi_event_group = xEventGroupCreate();

	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_init() STARTING WIFI_INIT_CONFIG_DEFAULT().\n");
    wifi_init_config_t cfg = WIFI_INIT_CONFIG_DEFAULT();
    esp_err_t err = esp_wifi_init(&cfg);
    if( err!=ESP_OK ) {
		ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_init() FAILED!\n");
	}
    ESP_ERROR_CHECK(err);

	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_init() STARTING esp_wifi_set_storage...\n");
    ESP_ERROR_CHECK(esp_wifi_set_storage(WIFI_STORAGE_RAM));
    ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_init() STARTING esp_wifi_set_mode(WIFI_MODE_NULL)!\n");
    ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_NULL));

	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_wifi_init() STARTING esp_wifi_start()!\n");
    esp_wifi_start();
...
{
	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_station_netif() STARTING\n");
    esp_netif_t *wifi_netif = NULL;
    wifi_mode_t mode = WIFI_MODE_NULL;

    if (data_forwarding || enable_dhcps) {
		ESP_LOGI(TAG, "DEBUG esp_bridge_create_station_netif() returning NULL!\n");
        return wifi_netif;
    }
...
{
	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() STARTING\n");
    esp_netif_ip_info_t netif_ip;
	...
	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() STARTING esp_bridge_wifi_init()\n");
    esp_bridge_wifi_init();

     wifi_ap_config_t ap_config;
    /* Get WiFi ap configuration */
    esp_wifi_get_config(WIFI_IF_AP, (wifi_config_t*)&ap_config);
    ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() AP ssid %s\n",     (const char*)ap_config.ssid);
    ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() AP password %s\n", (const char*)ap_config.password);
	
	//
	if( strlen(OVERRIDE_AP_SSID)>0 ) {
		ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() overriding SSID: %s & PWD: %s\n", OVERRIDE_AP_SSID, OVERRIDE_AP_PWD);
	...
	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() STARTING esp_netif_create_default_wifi_ap()\n");
    wifi_netif = esp_netif_create_default_wifi_ap();
	vTaskDelay(3000/portTICK_PERIOD_MS);
	esp_err_t err = esp_netif_dhcps_stop(wifi_netif);
	if( err!=ESP_OK ) {
		ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() FAILED esp_netif_create_default_wifi_ap()\n");
	}
    ESP_ERROR_CHECK(err);
    ...
    if (ip_info) {
        ...
        if (enable_dhcps) {
			ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() STARTING esp_bridge_netif_request_ip()\n");
    ...
    ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() got IP Address:" IPSTR"\n", IP2STR(&netif_ip.ip));
...
//<<<<<<< HEAD
    ...
    ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() NEW OPTION LITE_ENABLE STARTING!!!!\n");
...
//>>>>>>> master
	...
	ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() DONE!\n");
	if( wifi_netif==NULL ) {
		ESP_LOGI(TAG, "bridge_wifi.c -> esp_bridge_create_softap_netif() wifi_netif is NULL!\n");
	}
    //ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_AP, &wifi_config_ap));
